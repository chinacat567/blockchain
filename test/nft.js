...
describe('NFT', function () {
  ...
  it("increases token's id after each mint", async function () {
    await expect(nftContract.createNewMedicine(''))
  ...
  it('gets token ids owned by msg.sender', async function () {
    await nftContract.createNewMedicine('')
    await nftContract.connect(account2).createNewMedicine('')
    await nftContract.createNewMedicine('')
    await nftContract.transferFrom(account1.address, account2.address, 1)

    const nftIds = await nftContract.getMedicinesOwnedByMe()
  ...
  it('gets token ids created by msg.sender', async function () {
    ...
    const nftIds = await nftContract.getMedicinesCreatedBySender()
  ...
  it('gets token creator by id', async function () {
    await nftContract.createNewMedicine('')
    const creator = await nftContract.getMedicineCreator(1)
